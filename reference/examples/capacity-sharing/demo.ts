/**
 * Capacity Sharing Demo
 *
 * Demonstrates the ALXP capacity sharing model:
 * - Alice runs a Claude-backed agent, donates 50% of her monthly capacity
 * - Bob runs an Ollama agent, donates local GPU time
 * - Both earn credits; Alice uses Bob's GPU for drafts, Bob uses Alice's Claude for polish
 *
 * Run: npx tsx examples/capacity-sharing/demo.ts
 */

import { generateAgentIdentity } from "../../src/identity/did.js";
import { signString } from "../../src/identity/signing.js";
import { generateAgentCard, matchesQuery, calculateCreditCost } from "../../src/discovery/agent-card.js";
import { AgentRegistry } from "../../src/discovery/registry.js";
import { HeartbeatTracker, hasRemainingCapacity } from "../../src/discovery/heartbeat.js";
...
async function main() {
  ...
  // ── 1. Register agents with capacity sources ──
  ...
  // ── 2. Heartbeats with capacity snapshots ──
  ...
  // ── 4. Alice uses Bob's GPU for a fast draft ──
  ...
  // ── 5. Bob uses Alice's Claude for a polished version ──
  ...
  // ── 7. Discovery: find agents by provider ──
